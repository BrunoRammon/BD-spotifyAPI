...
def conditional_delete_from_table(item_name,table_name='track',by_column='name_track'):
    con = create_connection_spotify_db()
    cur = con.cursor()
    query = psycopg2.sql.SQL("DELETE FROM {table} WHERE {pkey} = %s;")\
        .format(table=psycopg2.sql.Identifier(table_name),
        pkey=psycopg2.sql.Identifier(by_column))
